...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
	return {
		...
		template: function(element, attrs) {
			...
			template += '<button ng-disabled="disabled" type="button" class="dropdown-toggle" ng-class="settings.buttonClasses" ng-click="toggleDropdown()" style="width:100%"><span style="float:left">{{getButtonText()}}</span><div style="float:right"><span class="caret"></span></div></button>';
        ...
        if ($scope.open) {
          $scope.close()
			...
			$scope.texts = {
				...
				disableSearch: 'Pesquisa desabilitada',
				enableSearch: 'Pesquisa habilitada',
				selectGroup: 'Todos selecionados:'
			};
